...
const updateCommnent = async (req: Request, res: Response) => {
  try {
    const authorId = req.user?.id as string;
    const { commentId } = req.params;
    const result = await commentService.updateCommnent(
      authorId,
      commentId,
      req.body,
    );
    ...
    res.status(500).json({ error: "Failed to comment update", details: error });
  }
};

const moderateCommnent = async (req: Request, res: Response) => {
  try {
    const { commentId } = req.params;
    const result = await commentService.moderateCommnent(commentId, req.body);
    res.status(200).json({
      success: true,
      data: result,
    });
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to comment update";
